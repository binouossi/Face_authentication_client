...
/*
void IplImagesender(int sock, IplImage frame)
{
    char buffer[999999];
    bzero (buffer,999999);
    ...
    if(n<0)
    {
        cout<<"IplImage send image send"<<endl;
    }
}
*/
                        ...
                        ,const char **argv)
{
    ...
    client* clin=new client(username);

  //  string jo=string(username);
...
//new comment
/* zeroFrame content the face image that where take
    that could be send to server to make the authentication
*/

// starting of verification



   // int val=newVerifier->verifyFace(zeroFrame);
/* la valeur de val est envoyer par le serveur*/

    int val=clin->communicator(zeroFrame);
    if (val==2)
    {
        ...
        if (queryImage!=0)
        {
            ...
            if (sqrt(pow(newDetector.eyesInformation.LE.x-newDetector.eyesInformation.RE.x,2) + (pow(newDetector.eyesInformation.LE.y-newDetector.eyesInformation.RE.y,2)))>28  && sqrt(pow(newDetector.eyesInformation.LE.x-newDetector.eyesInformation.RE.x,2) + (pow(newDetector.eyesInformation.LE.y-newDetector.eyesInformation.RE.y,2)))<120)
            {
                ...
                if (pow(ang,2)<200)
                {
                    ...
                    if (im!=0)
                    {
                        // ici aussi la valeur de val est direstement envoyer par le serveur*/
                        int val = clin->communicator(im);
...
int main()
{
    printf("kksldclsd");
    //pam_sm_authenticate();
    client* clin=new client("cani");

IplImage *hi=cvCreateImage( cvSize(IMAGE_WIDTH,IMAGE_HEIGHT),IPL_DEPTH_8U,3);
cvZero(hi);
int i=0,val;
